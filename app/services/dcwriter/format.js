...
function formCoords(valArr) {
    ...
    for (el in valArr) {
        ...
        if (valArr[el].includes('.')) valArr[el] = valArr[el].substring(0, valArr[el].indexOf('.') + 4)
        cStr += ' ' + valArr[el]
    }
    return cStr
}
...
exports.mines = async function mines(entry) {
    let attachment = new Discord.MessageAttachment(entry.file.path + entry.file.name, entry.file.name)
    let msg = {
