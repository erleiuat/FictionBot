...
async function sendMaps(dcClient) {
  ...
  let channel = dcClient.channels.cache.find(
    channel => channel.id === channels.maps
  )

  for (const e in global.newEntries.maps) {
    if (global.doSend)
      await channel.send(
        new Discord.MessageEmbed(await format.maps(global.newEntries.maps[e]))
      )
    dump[e] = {
      dump: 'maps',
...
async function sendMines(dcClient) {
  ...
  for (const e in global.newEntries.mines) {
    global.newEntries.mines[e].file = await mapLocation.generate(
      ...
      'mines_' +
        global.newEntries.mines[e].steamID +
        '_' +
        global.newEntries.mines[e].action +
        '_' +
        new Date().getTime() +
        '.jpg',
...
async function sendKills(dcClient) {
  ...
  let channel = dcClient.channels.cache.find(
    channel => channel.id === channels.kill
  )

  for (const e in global.newEntries.kill) {
    if (global.doSend)
      await channel.send(
        new Discord.MessageEmbed(await format.kill(global.newEntries.kill[e]))
      )
    ...
    global.newEntries.kill[e].file = await mapLocation.generate(
      ...
      'kill_' +
        global.newEntries.kill[e].Victim.UserId +
        '_' +
        new Date().getTime() +
        '.jpg',
      './app/storage/mapLocation/'
...
async function sendChats(dcClient) {
  ...
  let channel = dcClient.channels.cache.find(
    channel => channel.id === channels.chat
  )
  ...
  for (const el in global.newEntries.chat) {
    ...
    if (global.newEntries.chat[el].type == 'Global') {
      ...
      if (msg.fields[0].value.match(/(?:admin|support)/gim))
        if (global.doSend)
          await channel.send(' <@&' + process.env.DISCORD_ROLE_SUPPORT + '> ')
      if (global.doSend) await channel.send(new Discord.MessageEmbed(msg))
...
async function sendAdmins(dcClient) {
  ...
  let channel = dcClient.channels.cache.find(
    channel => channel.id === channels.admin
  )
  ...
  for (const el in global.newEntries.admin) {
    ...
    if (aList[line.steamID].useFakeNames) {
      if (msgCmd.includes('setfakename'))
        fakeNameCache[line.steamID] = line.message.split(': ')[1]
      else if (msgCmd.includes('clearfakename'))
        fakeNameCache[line.steamID] = false
    }
    ...
    if (!doHide) {
      ...
      if (global.doSend)
        await channel.send(
          new Discord.MessageEmbed(await format.admin(line, abuserID))
        )
...
async function sendLogins(dcClient) {
  ...
  let channel = dcClient.channels.cache.find(
    channel => channel.id === channels.login
  )
  ...
  for (const el in global.newEntries.login) {
    if (global.doSend)
      await channel.send(
        new Discord.MessageEmbed(
          await format.login(global.newEntries.login[el])
        )
      )
    ...
    if (
      !aList[global.newEntries.login[el].steamID] ||
      !aList[global.newEntries.login[el].steamID].hideLogin
    ) {
      if (global.newEntries.login[el].type == 'login')
...
async function sendDump(dcClient) {
  ...
  let channel = dcClient.channels.cache.find(
    channel => channel.id === channels.dump
  )

  for (const el in dump) {
    let msg = false

    if (dump[el].dump == 'kill')
      msg = {
      ...
      if (msg.fields[0].value.match(/(?:admin|support|abuse|abuze)/gim))
        if (global.doSend)
          await channel.send(' <@&' + process.env.DISCORD_ROLE_SUPPORT + '> ')
    } else if (dump[el].dump == 'admin')
      msg = {
        ...(await format.admin(dump[el])),
        color: '00ff00'
      }
    else if (dump[el].dump == 'login')
      msg = {
        ...(await format.login(dump[el])),
        color: '242424'
      }
    else if (dump[el].dump == 'mines')
      msg = {
