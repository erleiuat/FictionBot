    ...
    for (const logtype in logs) {
      ...
      for (const key in logs[logtype]) {
        if (!(key in logCache[logtype])) {
          ...
          global.log.debug(
            sn + 'Added new Entry for processing. (' + logtype + ')'
          )
      ...
      fs.writeFileSync(
        './app/storage/logs/' + logtype + '.json',
        JSON.stringify(logCache[logtype])
      )
    }
...
async function updateFTPCache() {
  ...
  do {
    ...
    try {
      ...
      if (global.updateCache)
        for (const key in logCache)
          await ftp.uploadFrom(
            './app/storage/logs/' + key + '.json',
            process.env.RM_LOG_FTP_DIR + key + '.json'
          )
...
async function getCurrentCache(logTypes) {
  ...
  let cacheContent = {}
  for (const key in logTypes)
    cacheContent[key] = JSON.parse(
      fs.readFileSync('./app/storage/logs/' + key + '.json')
    )
