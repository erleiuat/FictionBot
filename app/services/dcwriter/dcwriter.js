...
async function sendChats(dcClient) {
    ...
    for (const el in global.newEntries.chat) {
        ...
        if (global.newEntries.chat[el].type == 'Global') {
            ...
            if (msg.fields[0].value.match(/(?:admin|support)/gim)) await channel.send(' <@&' + process.env.DISCORD_ROLE_SUPPORT + '> ')
...
async function sendAdmins(dcClient) {
    ...
    for (const el in global.newEntries.admin) {
        ...
        else for (const cmd in aList[line.steamID].hideCommands) if (msgCmd.includes(cmd.toLowerCase())) doHide = true

        if (aList[line.steamID].alertCommands['#*']) doAlert = true
        else for (const cmd in aList[line.steamID].alertCommands) if (msgCmd.includes(cmd.toLowerCase())) doAlert = true
...
async function sendDump(dcClient) {
    ...
    for (const el in dump) {
        ...
        if (dump[el].dump == 'kill')
            ...
            if (msg.fields[0].value.match(/(?:admin|support|abuse|abuze)/gim)) await channel.send(' <@&' + process.env.DISCORD_ROLE_SUPPORT + '> ')
