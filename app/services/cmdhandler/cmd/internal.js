...
exports.mine_armed = async function mine_armed(cmd) {
    cmdBuilder.begin()
    let tmpCmd = cmdBuilder.getTmpCmd()
    cmdBuilder.addMessage('global', ':[Traps]: ・ If you have just placed a trap, please note that this is only allowed in and immediately around your Base. Remove the mine if this is not the case.')
    return cmdBuilder.fullCommand(tmpCmd)
}
...
exports.auth_log = async function auth_log(cmd) {
    cmdBuilder.begin()
    cmdBuilder.addMessage('global', ' ・ >> ' + cmd.user + ' ' + cmd.text + ' << ・ ')
    return cmdBuilder.fullCommand(cmd)
}
