...
async function iterateLists(dcClient) {
  let iteration = 1
  let chRanking = dcClient.channels.cache.find(
    channel => channel.id === process.env.DISCORD_CH_RANKING
  )
  let chStats = dcClient.channels.cache.find(
    channel => channel.id === process.env.DISCORD_CH_PLAYERSTATS
  )
  ...
  do {
    ...
    if (logins) {
      ...
      if (
        JSON.stringify(rankingPlaytime) != JSON.stringify(cache.rankingPlaytime)
      ) {
    ...
    if (logins2) {
      ...
      if (
        JSON.stringify(admPlayerstats) != JSON.stringify(cache.admPlayerstats)
      ) {
...
async function iterateOnline(dcClient) {
  let chOnline = dcClient.channels.cache.find(
    channel => channel.id === process.env.DISCORD_CH_PLAYERONLINE
  )
  ...
  do {
    ...
    list.push(
      '\n-----\n\n**Currently: ' +
        list.length +
        '**\n\n**Highscore: ' +
        (await statList.highscore(list.length)) +
        '**\n'
    )
...
async function iterateNewPlayers(dcClient) {
  let chNew = dcClient.channels.cache.find(
    channel => channel.id === process.env.DISCORD_CH_NEWPLAYERS
  )
...
async function mapTraps(dcClient) {
  let chTraps = dcClient.channels.cache.find(
    channel => channel.id === process.env.DISCORD_CH_MINES
  )
  ...
  do {
    ...
    if (
      trapList.active.length &&
      JSON.stringify(trapList) != JSON.stringify(cache.trapList)
    ) {
...
async function dcSend(msgs, channel) {
  ...
  for (const msg of msgs) {
    if (msg.length > 0) if (global.doSend) await channel.send(msg)
  }
}
